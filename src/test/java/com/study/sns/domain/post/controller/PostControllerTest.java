...
import com.study.sns.domain.post.dto.*;
...
public class PostControllerTest {
    ...
    void 게시글수정_실패_로그인하지않은_사용자() throws Exception {
        ...
        mockMvc.perform(patch("/api/v1/post/1")
                        .contentType(APPLICATION_JSON)
                        .content(objectMapper.writeValueAsBytes(req))).andDo(print())
                .andExpect(status().isUnauthorized());
    }
    ...
    void 게시글수정_실패_본인소유가아닌_게시물수정() throws Exception {
        ...
        // When
        when(postService.updatePost(any(UpdatePostServiceDto.class)))
                .thenThrow(new SnsApplicationException(PostErrorCode.INVALID_PERMISSION));
